...
\begin{document}
	...
	\begin{solution}{26}
		\begin{lemma}{Every graph $G$ with $\chi(G) = k$ has a proper $k$-coloring in which one color class has size at least $m=\alpha(G)$.}
			
		\end{lemma}
		\begin{lemma}{For every graph G $\chi(G) \leq |V(G)| - \alpha(G) + 1$}
		
		
		\end{lemma}
		\begin{lemma}{If $G = F \bigcup H$, then $\chi(G) \leq \chi(F) + \chi(H)$ } 
	...
	\begin{solution}{27}
		\begin{theorem}{For every Graph $G$ on $n$ vertices we have $\chi (G) + \chi (\bar{G}) \geq 2\sqrt{n}$}
		Using the lemma of small coloring results we know that $\chi (G)  \geq max (\omega (G) , \frac{n}{\alpha(G)})$ and that
