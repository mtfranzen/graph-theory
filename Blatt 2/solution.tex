...
\begin{document}
	...
	\begin{solution}{5}
		\begin{theorem}{Let $G$ be a nonempty graph with minimum degree at least two. Then, there is a connected graph $G'$ having the same degree sequence as $G$.}
			We will show that we are able to inter-connect any two connected components of $G$ without changing the graph's degree-sequence.\\ \ \\
			For any component $C_1 = (V_1, E_1)$ and any vertex $v_1 \in V_1$, we find at least two more vertices $v_2, v_3 \in V_1$ within $C_1$ which are adjacent to $v_1$. From these considerations, removing any edge will have no impact on the connectivity of some component of $G$.

%			Let $C_1 = (V_1, E_1)$ be a connected component of $G$. Since $G$ has degree at least two, we find at least two \emph{adjacent} vertices $v_1, v_2, v_3 \in V_1$. Respectively, let $w_1, w_2 \in V_2$ be two adjacent vertices of component $C_2 = (V_2, E_2)$. Furthermore, each of these vertices is adjacent to at least one \emph{other} vertex of it's component (again, since their degree is at least two).
		\end{theorem}
		\begin{figure}[h!]
