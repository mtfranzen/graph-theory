		...
		\end{align}}			
			...
			\begin{itemize}
				\item \textbf{Case 1: $G_i$ has at least one odd-degree vertex.}\\
					...
					\begin{itemize}
						\item \textbf{Case 1: $p$ is not a cycle.} Considering that $p$ is not a cycle, then the number of non-$p$-covered edges incident to $v_1$ is odd. As shown above, we find a walk $p' \in P$ ($p \neq p'$) starting in $v$. However, then we were able to decrease the amount of walks by fusing $p$ with $p'$. A contradiction to the minimum-size property of $P$. Hence, \emph{there is no even-degree vertex that is an endpoint of a cycle in $P$}.
