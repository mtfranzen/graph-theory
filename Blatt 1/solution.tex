...
\begin{document}
	...
	\begin{solution}{3}
		\begin{theorem}{Either a graph or its complement is connected.}
			Let $G = (V, E)$ be disconnected. Moreover, let $S = (V_S, E_S)$ and $T = (V_T, E_T)$ be two connected components of G ($S, T$ are not inter-connected).\\
			Thus, for any two vertices $s \in V_S$ and $t \in V_T$, there is no edge $\{s, t\}$ in $E$. On the other hand, $\{s, t\}$ is contained in the graphs complement $\bar{G}$.\\
			Moreover, for any additional vertex $t' \in V_T$, there exists an edge $\{s, t'\}$ and hence, the complement contains the path $(t, s, t')$.\\
			From these considerations, any two components that are not inter-connected in $G$ are inter-connected in $\bar{G}$ and any connected component of $G$ is connected in $\bar{G}$ as well.\\
			Since $S, T$ are chosen without any loss of generality, the complement of any disconnected graph is connected.\\
			\ \\
			Finally, if $\bar{G}$ is disconnected then $G$ is connected according to $\bar{\bar{G}} = G$.
		\end{theorem}
	\end{solution} 
	\newpage
	\begin{solution}{4}
